...
import { Edit2, Trash2, Plus, LogOut, FileText, Search, Loader2, List, Folder, Settings, Sun, Moon, ArrowLeft, Megaphone } from 'lucide-react';
...
const AdminDashboard = ({ user }) => {
    ...
    return (
        <div className="min-h-screen bg-neutral-50 p-6 md:p-12 animate-in fade-in transition-colors duration-500">
            <div className="max-w-6xl mx-auto">
                {/* Header */}
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
                    <div>
                        <h1 className="text-3xl font-black uppercase tracking-tighter text-neutral-900 mb-2">Painel Administrativo</h1>
                        <p className="text-sm text-neutral-500">Gerencie seus artigos e documentações</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => navigate('/')}
                            className="bg-white text-neutral-900 border border-neutral-200 px-4 py-3 rounded-lg hover:bg-neutral-50 transition-colors flex items-center gap-2"
                        ...
                        {activeTab === 'posts' && (
                            <button
                                ...
                                className="bg-neutral-900 text-white px-6 py-3 rounded-lg text-xs font-bold uppercase tracking-widest hover:opacity-90 transition-opacity flex items-center gap-2 shadow-lg"
                            >
                        ...
                        <button
                            ...
                            className="bg-white text-red-500 border border-neutral-200 px-4 py-3 rounded-lg hover:bg-red-50 transition-colors"
                            title="Sair"
                        >
                            <LogOut size={18} />
                        </button>
                ...
                <div className="flex gap-6 border-b border-neutral-200 mb-8 overflow-x-auto no-scrollbar">
                    <button
                        ...
                        className={`pb-4 whitespace-nowrap text-sm font-bold uppercase tracking-widest transition-all ${activeTab === 'posts' ? 'text-neutral-900 border-b-2 border-neutral-900' : 'text-neutral-400 hover:text-neutral-600'}`}
                    >
                        <span className="flex items-center gap-2"><List size={16} /> Artigos</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('categories')}
                        className={`pb-4 whitespace-nowrap text-sm font-bold uppercase tracking-widest transition-all ${activeTab === 'categories' ? 'text-neutral-900 border-b-2 border-neutral-900' : 'text-neutral-400 hover:text-neutral-600'}`}
                    >
                        <span className="flex items-center gap-2"><Folder size={16} /> Categorias</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('ads')}
                        className={`pb-4 whitespace-nowrap text-sm font-bold uppercase tracking-widest transition-all ${activeTab === 'ads' ? 'text-neutral-900 border-b-2 border-neutral-900' : 'text-neutral-400 hover:text-neutral-600'}`}
                    >
                        <span className="flex items-center gap-2"><Megaphone size={16} /> Anúncios</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('settings')}
                        className={`pb-4 whitespace-nowrap text-sm font-bold uppercase tracking-widest transition-all ${activeTab === 'settings' ? 'text-neutral-900 border-b-2 border-neutral-900' : 'text-neutral-400 hover:text-neutral-600'}`}
                    >
